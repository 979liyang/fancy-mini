...
如需引入，请参考[登录模块源码](../src/ZZLogin.js)，自行修改使用。    
...
ZZLogin.install({
    ...
    async userAuthHandler(){
      //先尝试直接出授权弹窗
      let userInfoRes = await wxResolve.getUserInfo({withCredentials: true});
      if (!userInfoRes.errMsg.includes('scope'))
        return userInfoRes;
  
      //若直接授权接口已废弃，则改用自定义弹窗授权
      ...
      userInfoRes = await page.$invoke('PageFrame/AuthModal', 'asyncOpen', { //展示自定义弹窗，并通过button获取用户信息
        title: tips.title,
...
注，登录模块不会去获取用户在转转的昵称、头像、加入时间、信用分等个人资料，如需使用请调用后端接口自行获取
*/
